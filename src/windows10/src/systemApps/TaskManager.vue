<template>
  <div class="windows10-system-task-manager">
    <div class="system-task-manager-content">
      <div
        class="system-task-manager-content-item"
        tabindex
        v-for="(app, index) in openAppList"
        :key="app.id"
        @click="selectIndex = index"
        :class="selectIndex === index ? 'select' : ''"
      >
        <div
          class="system-task-manager-content-ite-icon"
          v-html="app.config.icon"
        ></div>
        <div
          class="system-task-manager-content-ite-name"
          v-html="app.config.title"
        ></div>
      </div>
    </div>
    <div class="system-task-manager-footer">
      <button :disabled="!selectIndex && selectIndex !== 0" @click="close">
        结束任务
      </button>
    </div>
  </div>
