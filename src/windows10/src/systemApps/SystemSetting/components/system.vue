<template>
  <div class="windows10-system-set-individuation">
    <div class="system-set-individuation-title">系统设置</div>
    <div class="system-set-individuation-content">
      ...
      <div class="set-individuation-content-item title">背景</div>
      <div class="set-individuation-content-item method">
        <select
          v-model="individuation.background.type"
          @change="onChange"
          class="windwos10-select"
        >
      ...
      <div class="set-individuation-content-item title">
        <span
          v-text="
            individuation.background.type === 'image'
              ? '选择背景图片'
              : '选择背景颜色'
          "
        ></span>
      ...
      <div class="set-individuation-content-item method">
        <div
          class="individuation-images"
          v-show="individuation.background.type === 'image'"
        >
          <div
            class="individuation-image-item"
            v-for="(bg, index) in backgroundList"
            :key="index"
            :style="bgItemStyle(bg)"
            :class="individuation.background.backgroundImage === bg ? 'on' : ''"
            @click="setBackground(bg)"
          ></div>
        </div>
        <input
          v-show="individuation.background.type === 'color'"
          type="color"
          v-model="individuation.background.backgroundColor"
          class="windwos10-color-select"
          @input="onChange"
        />
      </div>
      <div class="set-individuation-content-item title">字体颜色</div>
      <div class="set-individuation-content-item method">
        <input
          type="color"
          v-model="individuation.color"
          class="windwos10-color-select"
          @input="onChange"
        />
      </div>
      <div class="set-individuation-content-item title">图标大小</div>
      <div class="set-individuation-content-item method">
...
export default {
  ...
  computed: {
    backgroundList() {
      return [desktopImage0, desktopImage1, desktopImage2].concat(
        this.individuation.backgroundList
      );
    },
    miniDesktop() {
      const style = {};
